"""Compare draw environment exports against asm and the pinned EXE."""
...
def test_draw_env_exports_match_asm_and_pinned_exe():
    ...
    manifest = json.loads((ROOT / "artifacts/draw-env-source-exports.json").read_text())
    assert manifest["status"] == "SOURCE_EXPORTS_PUBLISHED"
    assert manifest["total_words"] == 93
    ...
    body = (ROOT / "src/main/80014960.c").read_text().split("#else\n", 1)[1]
    assert "void func_80014960(void)" in body and "MUSASHI_NATIVE_MIPS_WORD" not in body

    preserved = manifest["exports"][2]["preserved_c_sha256"]
    assert hashlib.sha256(body.removesuffix("#endif\n").encode()).hexdigest() == preserved
