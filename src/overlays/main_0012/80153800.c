/* Overlay range [80153800,80153978) from MAIN.CD member 0012.
 * SHA256(span)=ccd049fe8f32081141aa0447ee97b9355e7c6abf9791397cec365d3a6a4cf4fa.
 * Word export for the native seam; the body below is an
 * UNVERIFIED draft, not an oracle match claim. */
#ifdef MUSASHI_NATIVE_MIPS_WORD_EXPORT
MUSASHI_NATIVE_MIPS_WORD(0x3C028012)
MUSASHI_NATIVE_MIPS_WORD(0x8C42DB28)
MUSASHI_NATIVE_MIPS_WORD(0x27BDFFD0)
MUSASHI_NATIVE_MIPS_WORD(0xAFB10024)
MUSASHI_NATIVE_MIPS_WORD(0x00808821)
MUSASHI_NATIVE_MIPS_WORD(0xAFBF002C)
MUSASHI_NATIVE_MIPS_WORD(0xAFB20028)
MUSASHI_NATIVE_MIPS_WORD(0x1040000E)
MUSASHI_NATIVE_MIPS_WORD(0xAFB00020)
MUSASHI_NATIVE_MIPS_WORD(0x26240004)
MUSASHI_NATIVE_MIPS_WORD(0x0C00565E)
MUSASHI_NATIVE_MIPS_WORD(0x26250088)
MUSASHI_NATIVE_MIPS_WORD(0x8A22008B)
MUSASHI_NATIVE_MIPS_WORD(0x9A220088)
MUSASHI_NATIVE_MIPS_WORD(0x8A23008F)
MUSASHI_NATIVE_MIPS_WORD(0x9A23008C)
MUSASHI_NATIVE_MIPS_WORD(0xAA220093)
MUSASHI_NATIVE_MIPS_WORD(0xBA220090)
MUSASHI_NATIVE_MIPS_WORD(0xAA230097)
MUSASHI_NATIVE_MIPS_WORD(0xBA230094)
MUSASHI_NATIVE_MIPS_WORD(0x08054E57)
MUSASHI_NATIVE_MIPS_WORD(0x00001021)
MUSASHI_NATIVE_MIPS_WORD(0x922300DE)
MUSASHI_NATIVE_MIPS_WORD(0x00000000)
MUSASHI_NATIVE_MIPS_WORD(0x2C620021)
MUSASHI_NATIVE_MIPS_WORD(0x1440001C)
MUSASHI_NATIVE_MIPS_WORD(0x00031880)
MUSASHI_NATIVE_MIPS_WORD(0x3C108012)
MUSASHI_NATIVE_MIPS_WORD(0x26106DB0)
MUSASHI_NATIVE_MIPS_WORD(0x02002021)
MUSASHI_NATIVE_MIPS_WORD(0x0C005655)
MUSASHI_NATIVE_MIPS_WORD(0x26250004)
MUSASHI_NATIVE_MIPS_WORD(0x8A020003)
MUSASHI_NATIVE_MIPS_WORD(0x9A020000)
MUSASHI_NATIVE_MIPS_WORD(0x8A030007)
MUSASHI_NATIVE_MIPS_WORD(0x9A030004)
MUSASHI_NATIVE_MIPS_WORD(0xAA22008B)
MUSASHI_NATIVE_MIPS_WORD(0xBA220088)
MUSASHI_NATIVE_MIPS_WORD(0xAA23008F)
MUSASHI_NATIVE_MIPS_WORD(0xBA23008C)
MUSASHI_NATIVE_MIPS_WORD(0x8A22008B)
MUSASHI_NATIVE_MIPS_WORD(0x9A220088)
MUSASHI_NATIVE_MIPS_WORD(0x8A23008F)
MUSASHI_NATIVE_MIPS_WORD(0x9A23008C)
MUSASHI_NATIVE_MIPS_WORD(0xAA220093)
MUSASHI_NATIVE_MIPS_WORD(0xBA220090)
MUSASHI_NATIVE_MIPS_WORD(0xAA230097)
MUSASHI_NATIVE_MIPS_WORD(0xBA230094)
MUSASHI_NATIVE_MIPS_WORD(0x8E240020)
MUSASHI_NATIVE_MIPS_WORD(0x3C038012)
MUSASHI_NATIVE_MIPS_WORD(0x94636DB6)
MUSASHI_NATIVE_MIPS_WORD(0x00001021)
MUSASHI_NATIVE_MIPS_WORD(0x08054E57)
MUSASHI_NATIVE_MIPS_WORD(0xA4830012)
MUSASHI_NATIVE_MIPS_WORD(0x00609021)
MUSASHI_NATIVE_MIPS_WORD(0x962200A2)
MUSASHI_NATIVE_MIPS_WORD(0x00008021)
MUSASHI_NATIVE_MIPS_WORD(0x00431023)
MUSASHI_NATIVE_MIPS_WORD(0xA7A2001A)
MUSASHI_NATIVE_MIPS_WORD(0x00102540)
MUSASHI_NATIVE_MIPS_WORD(0x00042403)
MUSASHI_NATIVE_MIPS_WORD(0x27A50010)
MUSASHI_NATIVE_MIPS_WORD(0x00A03021)
MUSASHI_NATIVE_MIPS_WORD(0xA7A00010)
MUSASHI_NATIVE_MIPS_WORD(0xA7A00012)
MUSASHI_NATIVE_MIPS_WORD(0x0C004E72)
MUSASHI_NATIVE_MIPS_WORD(0xA7B20014)
MUSASHI_NATIVE_MIPS_WORD(0x962200A0)
MUSASHI_NATIVE_MIPS_WORD(0x97A30010)
MUSASHI_NATIVE_MIPS_WORD(0x02202021)
MUSASHI_NATIVE_MIPS_WORD(0x00431021)
MUSASHI_NATIVE_MIPS_WORD(0xA7A20018)
MUSASHI_NATIVE_MIPS_WORD(0x962200A4)
MUSASHI_NATIVE_MIPS_WORD(0x97A30014)
MUSASHI_NATIVE_MIPS_WORD(0x27A50018)
MUSASHI_NATIVE_MIPS_WORD(0x00431021)
MUSASHI_NATIVE_MIPS_WORD(0x0C054E5E)
MUSASHI_NATIVE_MIPS_WORD(0xA7A2001C)
MUSASHI_NATIVE_MIPS_WORD(0x1040FFC5)
MUSASHI_NATIVE_MIPS_WORD(0x26100001)
MUSASHI_NATIVE_MIPS_WORD(0x2A020080)
MUSASHI_NATIVE_MIPS_WORD(0x1440FFEA)
MUSASHI_NATIVE_MIPS_WORD(0x00102540)
MUSASHI_NATIVE_MIPS_WORD(0x922300DE)
MUSASHI_NATIVE_MIPS_WORD(0x24020001)
MUSASHI_NATIVE_MIPS_WORD(0x24630001)
MUSASHI_NATIVE_MIPS_WORD(0xA22300DE)
MUSASHI_NATIVE_MIPS_WORD(0x8FBF002C)
MUSASHI_NATIVE_MIPS_WORD(0x8FB20028)
MUSASHI_NATIVE_MIPS_WORD(0x8FB10024)
MUSASHI_NATIVE_MIPS_WORD(0x8FB00020)
MUSASHI_NATIVE_MIPS_WORD(0x27BD0030)
MUSASHI_NATIVE_MIPS_WORD(0x03E00008)
MUSASHI_NATIVE_MIPS_WORD(0x00000000)
#else
/* Body below is an UNVERIFIED draft, not an oracle match
 * claim; promotion requires tools/match_function.py MATCH. */
#include "psx_types.h"

/* m2c draft from main_0012.s: NOT verified against retail. C89-gated only;
 * promotion requires an oracle MATCH (tools/match_function.py). Types
 * and signatures are whatever the decompiler guessed; they are not
 * evidence of the original declaration. */

void func_800139C8(s32, u16 *, u16 *);                 /* extern */
void func_80015954(void *, void *);                       /* extern */
void func_80015978(s32, s32);                          /* extern */
s32 func_80153978(void *, s16 *);                   /* static */
extern s32 D_8011DB28;
extern s32 *D_80126DB0;
extern u16 D_80126DB6;

s32 func_80153800(void *arg0) {
    s16 sp1C;
    s16 sp1A;
    s16 sp18;
    u16 sp14;
    s16 sp12;
    u16 sp10;
    s32 var_a0;
    s32 var_s0;
    u16 temp_v1_2;
    u8 temp_v1;

    if (D_8011DB28 != 0) {
        func_80015978(arg0 + 4, arg0 + 0x88);
        arg0->unk93 = (unaligned s32) M2C_ERROR(/* Unable to handle lwr; missing a corresponding lwl */);
        arg0->unk97 = (unaligned s32) M2C_ERROR(/* Unable to handle lwr; missing a corresponding lwl */);
        goto block_2;
    }
    temp_v1 = arg0->unkDE;
    temp_v1_2 = temp_v1 * 4;
    if (temp_v1 >= 0x21U) {
        func_80015954(&D_80126DB0, arg0 + 4);
        arg0->unk8B = (unaligned s32) M2C_ERROR(/* Unable to handle lwr; missing a corresponding lwl */);
        arg0->unk8F = (unaligned s32) M2C_ERROR(/* Unable to handle lwr; missing a corresponding lwl */);
        arg0->unk93 = (unaligned s32) M2C_ERROR(/* Unable to handle lwr; missing a corresponding lwl */);
        arg0->unk97 = (unaligned s32) M2C_ERROR(/* Unable to handle lwr; missing a corresponding lwl */);
        arg0->unk20->unk12 = (u16) D_80126DB6;
        return 0;
    }
    var_s0 = 0;
    sp1A = arg0->unkA2 - temp_v1_2;
    var_a0 = 0 << 0x15;
loop_6:
    sp10 = 0;
    sp12 = 0;
    sp14 = temp_v1_2;
    func_800139C8(var_a0 >> 0x10, &sp10, &sp10);
    sp18 = arg0->unkA0 + sp10;
    sp1C = arg0->unkA4 + sp14;
    var_s0 += 1;
    if (func_80153978(arg0, &sp18) != 0) {
        var_a0 = var_s0 << 0x15;
        if (var_s0 >= 0x80) {
            arg0->unkDE = (u8) (arg0->unkDE + 1);
            return 1;
        }
        goto loop_6;
    }
block_2:
    return 0;
}
#endif
